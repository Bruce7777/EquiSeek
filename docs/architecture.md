...
The desktop investment page adds a business-scoped long-running harness rather than embedding an unbounded general code agent. `InvestmentAgentRuntime` runs a bounded lead-agent loop over registered portfolio, evidence, macro, market-analysis, screening, optional Tavily search, and permission-controlled tools inside a workspace explicitly selected by the user. It progressively activates built-in or user-overridden Skills, writes paired tool events and a Markdown report, and uses compressed investment context. DeepSeek may select the next structured action; the same boundary works with a deterministic local planner. The selected workspace can expose a persistent sandboxed shell and file tools according to the chosen permission mode; arbitrary host-file access, credentials, and raw portfolio cost fields remain excluded. Local persistence remains SQLite plus per-run files; PostgreSQL is only an optional future multi-worker coordination backend.
...
The desktop investment loop adds a deterministic `AgentLoopHarness` control plane around model selection and `ToolPipeline` execution. It owns a revisioned plan with at most one `in_progress` item, distinguishes runtime-owned fallback plans from model-owned full-plan snapshots, and never treats one successful tool call as proof that a model-owned research task is complete. The policy-filtered tool catalog receives a stable SHA-256 fingerprint. Only initially promoted tool schemas enter a model request; deferred tools remain visible as a lightweight name, description, and risk catalog and can be promoted by purpose or exact `select:tool.name` lookup without expanding `PolicySnapshot` authority.

Model-visible observations have per-item and total character budgets. Oversized observations are preserved under `.state/tool-results`, while the request receives a summary, preview, relative path, file-content SHA-256, and size. The always-promoted read-only `tool_results.read` tool can page through only the current Run's regular result files, with a maximum 5,000-character page; traversal, missing files, and symbolic links fail closed. This prevents output loss without creating a persist/read/persist loop. Consecutive identical tool calls use canonical arguments and produce advisory reminders at calls 3, 5, and 8; they are not rejected solely for repetition. Unknown and unpromoted tools remain hard failures.

Harness summaries and ordinary tool Trace entries prefer the append-only event stream over mutable in-memory state. They cite the relevant event sequence and `.state/events.jsonl`, while snapshots remain a compatibility fallback. Trace records auditable actions, policies, evidence references, and outcomes; it does not expose hidden model reasoning.
