...
function AgentWorkspace({ bootstrap, selectedSkills, onRun, onWorkspaceChange, onSettingsChange }: { bootstrap: BootstrapData; selectedSkills: Set<string>; onRun: (run: RunView | null, goal: string) => void; onWorkspaceChange: (items: WorkspaceSummary[]) => void; onSettingsChange: (settings: Record<string, unknown>) => void }) {
  ...
  useEffect(() => api.runs.subscribe(async (event) => {
    ...
    if (event.type === 'run.succeeded') {
      ...
      const workspaceInfo = (result.workspace_context || {}) as AnyRecord;
      setMessages((current) => [...current, { role: 'assistant', text: String(result.answer || '任务已完成。'), runId: run.runId, meta: `${result.answer_mode === 'local' ? '本地确定性规划' : String(modelInfo.id || model)} · ${result.tool_calls?.length || 0} 个工具步骤${workspaceInfo.name ? ` · ${workspaceInfo.name}` : ''}${result.vision?.used ? ' · 已使用图文模型' : ''}` }]);
      void refreshThreads();
    }
  ...
  return (
    <div className="agent-page">
      <header className="agent-header">
        ...
        <div className="mode-badge"><CircleDot size={13} /><span>{bootstrap.settings.enableNetwork ? '联网研究已开启' : '离线模式'} · {bootstrap.settings.enableDeepSeek ? modelOptions.find((item) => item.id === model)?.name || model : '本地规则模式'}</span></div>
      </header>
